function [c,ceq] = obstacle_constraints(x,Xobs,Ndec,PredHorizon,n,m)
%% Find centerline of obstacles
for pp=1:length(Xobs)
x_cline(pp,:)=[mean(Xobs{pp}(1:2,1)) mean(Xobs{pp}(3:4,1))];
y_cline(pp,:)=[mean(Xobs{pp}(2:3,2)) mean(Xobs{pp}([4 1],2))];
q_centroid(pp,:)=[mean(Xobs{pp}(:,1)) mean(Xobs{pp}(:,2))];

%% Find if current trajectory is close to obstacle
x_locs=x(1:n:n*(PredHorizon+1)); y_locs=x(3:n:n*(PredHorizon+1));
[mindist,ind]=min(((x_locs-q_centroid(pp,1)).^2+(y_locs-q_centroid(pp,2)).^2));
...
for kk=1:(PredHorizon+1)
    ...
    xc1=Xobs; yc1=TestTrack.cline(2,ind(1));
