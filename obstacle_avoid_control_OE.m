...
for pp=1:nobs
    ...
    plot(Xobs{pp}(3:4,1),Xobs{pp}(3:4,2),'-r')
    plot(Xobs{pp}([4 1],1),Xobs{pp}([4 1],2),'-g')
end
...
xsize = (horizon+1)*n; 
Q = eye(2,2);%diag(repmat((ones(1,6)./[100 10 100 10 0.1 0.01]).^2,1,horizon+1));%eye(xsize);%
...
%     z = quadprog(H, f, Aineq, bineq, Aeq, beq); 
    ...
    nlcons=@(var)track_nlcons(var,TestTrack,Ndec,horizon,n,m);%Nonlinear track constraints
